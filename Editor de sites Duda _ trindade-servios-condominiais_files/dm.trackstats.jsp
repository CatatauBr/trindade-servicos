    ...
    var MIXPANEL_CUSTOM_LIB_URL = "https://mp.multiscreensite.com/lib.min.js";
    ...
    const mpSettings = {
        ...
        'record_mask_text_selector': '#stripePopupWrapper,[autocomplete^=cc-],[data-auto="client-info-cell"],[data-auto="client-profile-body"],#add_client_popup_keyWrapper,[data-tab-id="submission"],#deleteFormResponses,#stripePopupWrapper',
        'record_mask_input_selector': 'input[type=password],input[type=hidden],input[autocomplete^=cc-]',
        'record_heatmap_data': window._flags['mixpanel.recording.heatmap.enabled']
    };
    
    mpSettings['api_host'] = 'https://mp.multiscreensite.com';
    

    mixpanel.init("95ad53f6a4af65dcf1815ef6bff3ca19", mpSettings);
    

    const recordingEnabled =
        window._flags?.['mixpanel.recording.enabled'] ||
        [
            { flag: 'mixpanel.recording.dashboard.enabled', path: 'home/dashboard' },
            { flag: 'mixpanel.recording.widgetBuilder.enabled', path: 'home/widgetBuilder' }
        ].some(({ flag, path }) =>
            window._flags?.[flag] && window.location.href.includes(path)
        ) ||
        (window._flags?.['mixpanel.recording.dashboard.enabled'] && window.location.pathname.endsWith('/home'))
    ...
    var _trackCreatedViaIframe = true;
    ...
    function wrTrackEvent(event, params) {
        ...
        var productType = "", isFromScratch = "", siteAlias = "", externalUID = null;
        ...
        if ($.dmx && $.dmx.site) {
            ...
            externalUID = $.dmx.site.externalUID;
        }
        params = $.extend({
            product: productType,
            isBFS: isFromScratch,
...
// DO NOT ADD GLOBAL VARIABLES, prefer passing it with the params since initHallDashboard.jsp does not bring all the global variables as commonHead.jsp do.
    function edTrackEvent(event, params) {
        ...
        var externalUID = null;
        var isFlexEditor = false;
        var siteStatus = null;
        var siteCreationMethod = null;
        var blogId = null;
        ...
        if ($.dmx && $.dmx.site) {
            ...
            isPublished = $.dmx.site.isPublished;
            externalUID = $.dmx.site.externalUID;
            siteStatus = $.dmx.site.status;
            siteCreationMethod = $.dmx.site.creationMethod || null;
            blogId = $.dmx.site.blogId;
        }
        if ($.dmx && $.dmx.siteRuntimeFlags) {
            isFlexEditor = $.dmx.siteRuntimeFlags.FLEX_SITE;
        }
        params = $.extend({
            ...
            externalUID: externalUID,
            isFlexEditor: isFlexEditor,
            'site-status': siteStatus ? (siteStatus === 'IN_PLANNING' ? 'Planning' : 'Site') : null,
            'site-creation-method': siteCreationMethod,
            hasBlog: !!blogId
        }, params);
        dmTrackEvent(event, "siteAlias", siteAlias, params);
    }
    ...
    function dmTrackNewUser(alias) {
        try {
            ...
            mixpanel.instance2?.alias(alias);
        } catch (e) {
        }
    }
    ...
    function dmTrackSuperProperty(json) {
        ...
        mixpanel.instance2?.register(json);
    }
    ...
    function dmTrackIdentity({uuid, name, isReseller, isCustomer, isProTrial, moreProperties}) {

        try {

            mixpanel.identify(uuid);
            mixpanel.instance2?.identify(uuid);
            mixpanel.name_tag(uuid);
            mixpanel.instance2?.name_tag(uuid);
            // This is important for backwards compatability - we alias the old id we were using (name) to the new id we are now using (uuid)
            mixpanel.alias(uuid, name);
            mixpanel.instance2?.alias(uuid, name);
            ...
            mixpanel.instance2?.register({
                'user_type': userType,
                'isInProTrial': isProTrial ? true : false
            });
            var sessionTracking = getInitialMixpanelSessionData(name);
            mixpanel.register({sessionNumber: sessionTracking.sessionNumber});
            mixpanel.instance2?.register({sessionNumber: sessionTracking.sessionNumber});
            mixpanel.people.set({$email: name, $name: name, uuid: uuid, user_type: userType, 'isInProTrial': isProTrial ? true : false});
            mixpanel.instance2?.people?.set({$email: name, $name: name, uuid: uuid, user_type: userType, 'isInProTrial': isProTrial ? true : false});


            if (moreProperties) {
                mixpanel.register(moreProperties);
                mixpanel.instance2?.register(moreProperties);
                mixpanel.people.set(moreProperties);
                mixpanel.instance2?.people?.set(moreProperties);
                if (moreProperties.accountCreationDate) {
                    mixpanel.people.set({$created: moreProperties.accountCreationDate});
                    mixpanel.instance2?.people?.set({$created: moreProperties.accountCreationDate});
                }
            }
...
function dmTrackEvent(event, param, value, additionalParams) {
	...
	updateMixPanelSession(event, param, value, additionalParams);
		...
		if (param == undefined) {
			...
			mixpanel.instance2?.track(event);
		} else {
			var params = {};
			params[param] = value;
			if (additionalParams) {
			...
			var dmAccount = dmReadCookie('_dm_account');
			...
			mixpanel.instance2?.track(event, params);
...
function updateMixPanelSession(event, param, value, additionalParams) {
	try {
		var sessionInterval = parseInt(
				(window.commonProps && window.commonProps['mixpanel.session.tracking.interval'])
				|| 30 * 60 * 1000, 10);
		var id = window.dm_account_name || (window.hallDashboardData
				|| { account: {} }).account.name || 'defaultAccount';
		var sessionData = JSON.parse(window.localStorage.getItem('builderSessionData'));
		if (!sessionData) {
			return;
		}
		...
		if (currentTime - sessionData[id].lastEvent > sessionInterval) {
			sessionData[id].sessionNumber = sessionData[id].sessionNumber + 1;
			window.mixpanel.register({ sessionNumber: sessionData[id].sessionNumber });
			window.mixpanel.instance2?.register({ sessionNumber: sessionData[id].sessionNumber });
		}
		sessionData[id].lastEvent = currentTime;
